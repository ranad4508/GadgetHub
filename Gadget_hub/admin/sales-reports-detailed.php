...
<?php
$ret=mysqli_query($con,"select year(OrderTime) as lyear,sum(ItemPrice) as totalitmprice from tblorders join tblorderaddresses on tblorderaddresses.Ordernumber=tblorders.OrderNumber join tblitem on tblitem.ID=tblorders.ItemId where year(tblorderaddresses.OrderTime) between '$fdate' and '$tdate' and tblorderaddresses.OrderFinalStatus='$fstatus' group by lyear");
$cnt=1;
while ($row=mysqli_fetch_array($ret)) {
